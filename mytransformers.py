...
class TransformerDecoder(Module):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None,
                ...
                need_weights: Optional[bool] = False, layer_outputs=False) -> Tensor:
        ...
        attn2s = []
        for mod in self.layers:
            output, attn, attn2 = mod(output, memory, tgt_mask=tgt_mask,
            ...
            attns.append(attn)
            attn2s.append(attn2)
        attns = torch.stack(attns, 0) #[n_layer, batch, L, S]
        attn2s = torch.stack(attn2s, 0)
        if layer_outputs:
            output = torch.stack(outputs, 0)
        if self.norm is not None:
            output = self.norm(output)

        return output, attns, attn2s
