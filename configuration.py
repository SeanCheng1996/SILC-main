...
class Config(dict):
    def version_config(self, version, train_v):
        hp = {
            1: {'dataset': 'IU', 'lr': 3e-5, 'vbackbone': 'resnet101', 'batch': 32, 'vb': 32, 'cs': True, 'MRG': True,
                'VQ': 1024, 'normal_gen': False, 'normal_c': False, 'nes': True},
            1.1: {'dataset': 'IU_l', },
            1.2: {'dataset': 'IU_f'},
            1.3: {'mv_fuse': 'max'},
            1.4: {'nes': True, 'transfer_param': ['image_encoder'], 'n_epoch': 200},

            2: {'dataset': 'MIMIC', 'lr': 3e-5, 'vbackbone': 'resnet101', 'batch': 32, 'vb': 32, 'cs': True,
                'MRG': True, 'VQ': 1024, 'n_epoch': 8, 'normal_gen': False, 'normal_c': False, 'nes': True},
        }
        for v in hp:
        ...
        if 'MIMIC' in self['dataset']:
            ...
            self['report_dir'] = 'D:/work/projects/SILC-main/preprocess/IU_xray'  # todo1
            self['img_dir'] = 'D:/work/projects/data/IU_xray/images/images_normalized/{uid}_IM-*.dcm.png'  # todo1
        ...
        train_hp = {0: {},
                    1: {'batch': 32, 'stage': 7, 'lr': 1e-6, 'RL_first': True, 'RL_first_only': True,
                        'RL_first_direct': True, 'cider_w': 1, 'cider_n': 1, 'bleu_w': 0, 'bleu_n': 0,
                        'n_epoch': 5 if 'MIMIC' in self['dataset'] else 100},
                    2: {'batch': 8, 'stage': 4, 'lr': 1e-6, 'RL_first': False, 'cider_w': 0, 'bleu_w': 1,
                        'bleu_n': (1, 2, 3, 4), 'n_epoch': 1 if 'MIMIC' in self['dataset'] else 50},
                    3: {'batch': 8, 'stage': 4, 'lr': 1e-6, 'RL_first': False, 'cider_w': 1, 'bleu_w': 1,
                        'bleu_n': (1, 2, 3, 4), 'n_epoch': 1 if 'MIMIC' in self['dataset'] else 50},
                    4: {'batch': 8, 'stage': 4, 'lr': 1e-6, 'RL_first': False, 'cider_w': 1, 'bleu_w': 0, 'cider_n': 4,
                        'n_epoch': 1 if 'MIMIC' in self['dataset'] else 50},
                    }
