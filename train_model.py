...
from dataset import XRayDataset, Tokenizer, CompatibleLlamaTokenizer
from configuration import Config
from models import ReportGenerator_contrast, VQ
from losses import CE, contrast_loss, contrast_loss_i2s, contrast_loss_s2i, RewardCriterion, get_self_critical_reward, \
    clear_scorer
...
def compute_batch(model, source, targets, verbose=False, optional_ret=[], model_optional=[],
    ...
    if not RL:  # Joint training of the whole model. Does not involve reinforcement learning.
        ...
        if conf['MRG']:
            max_sen_num = torch.max(targets['seg_s']).long().item() - 100 - (
                0 if conf['no_empty_sentence'] else 1)  # batch中所有报告最大句子数。要弄成int而不是tensor，否则多卡时一个tensor喂给多个卡会出问题
        ...
        if conf['RL_first'] and conf['MRG']:  # Train the topic decoder
            ...
            if conf['RL_first_direct']:  # Directly optimize the predicted topic tokens
                eos = eos.cpu().numpy()
                reward, real_reward = get_self_critical_reward(greedy_sen_token, gt_sen_token.cpu().numpy(),
                                                               sample_sen_token, conf['cider_weight'],
                                                               conf['bleu_weight'], bleu_n=conf['bleu_n'],
                                                               tokenizer=None, eos=eos, sos=10000, corpus_path=str(
                        Path(conf['model_dir']).parent) + '/mimic.p' if not conf['no_corpus'] else 'corpus',
                                                               cider_n=conf['cider_n'])  # [b]
                reward = torch.from_numpy(reward).float().to(sample_sen_token.device)
                losses['loss_RL_first'] = RewardCriterion(sample_sen_probs, sample_sen_token,
                                                          reward.unsqueeze(1).repeat(1, sample_sen_token.shape[1]),
                                                          ignore_zero=False)
        ...
        if not conf['RL_first_only']:  # Train the word decoder
            greedy_pred, sample_pred, sample_probs = greedy_pred[:, 1:], sample_pred[:, 1:], sample_probs[:,
                                                                                             1:]  # ignore the SOS token
            if not conf['MRG']:
                gts = targets['find'].cpu().numpy()
            else:
                caption = targets['find_s'].cpu().numpy()
                gts = np.zeros((caption.shape[0] * max_sen_num, caption.shape[1]),
                               dtype=np.int32)  # 一定要int，否则后面转str会和pred不一样
            ...
            reward, real_reward = get_self_critical_reward(greedy_pred, gts, sample_pred, conf['cider_weight'],
                                                           conf['bleu_weight'],
                                                           sigma=6.0, bleu_n=conf['bleu_n'], tokenizer=None,
                                                           corpus_path=corpus_path,
                                                           cider_n=conf['cider_n'],
                                                           corpus_ids=gt_sen_token.flatten().cpu().numpy().tolist() if
                                                           conf['separate_corpus'] else None)  # [B]
            reward = torch.from_numpy(reward).float().to(sample_pred.device)
            # print('reward', reward.shape, '\n', reward)
            losses['loss_RL'] = RewardCriterion(sample_probs, sample_pred,
                                                reward.unsqueeze(1).repeat(1, sample_pred.shape[1]),
                                                ignore_zero=False)  # 2023.11.2在0_1 R55训练，出现所有句子都是0，ignore_zero改为False
...
def evaluate(model, loader, tokenizer, output=False, n=10000, output_topic=False, beam_size=1, max_l=100,
             repeat_single=True):
    """
...
def get_model():
    return ReportGenerator_contrast(n_token=conf['n_token'], max_l=conf['max_l'],
                                    ...
                                    n_fuse_layer=conf['n_fuse_layer'], real_n_content=conf['real_n_content'],
                                    normal_gen=conf['normal_gen'],
...
def get_dataset(split, tokenizer, sample_lv=False):
    ...
    dataset = XRayDataset(report_dir=conf['report_dir'],
                          ...
                          nes=conf['no_empty_sentence'], delimiter=conf['delimiter'],
                          augment=(split == 'train' and conf['augment']),
                          image_size=conf['image_size'], image_pad=conf['image_pad'],
                          sample_lv=(sample_lv if sample_lv else (conf['sample_lv'] if split == 'train' else False)))
    return dataset
...
def train(resume=None, stage=1, RL=False, reset_epoch=False, transfer=False):
    ...
    tokenizer = CompatibleLlamaTokenizer("./tokenizer", conf['max_l'])
    ...
    train_loader = DataLoader(train_data, batch_size=conf['batch_size'], shuffle=True,
                              num_workers=min(0, conf['minibatch']), drop_last=True)
    valid_loader = DataLoader(valid_data, batch_size=conf['valid_batch'], shuffle=True,
                              num_workers=min(0, conf['minibatch']), drop_last=True)
    ...
    if stage == 1:
        ...
        if torch.cuda.device_count()>1:
            ...
            backbone = list(map(id, model.image_encoder.parameters()))
            others = list(filter(lambda p: id(p) not in backbone, model.parameters()))
            optimizer = torch.optim.Adam(
                [{'params': model.image_encoder.parameters(), 'lr': conf['lr_backbone']},
                 {'params': others, 'lr': conf['lr']}],
                betas=conf['adam_beta'], weight_decay=conf['weight_decay'])
    elif stage == 4:  # 用在RL时
        ...
        for (source, targets, debug) in tqdm(train_loader):
            ...
            if (RL and step.reach_cycle(100)) or (not RL and step.reach_cycle(1000)):
                ...
                if pred.dim() == 2:  # RL返回的[B,L]
                    ...
                    logger.log('|'.join(
                        [tokenizer.decode(pred[0, i].cpu().numpy(), ['SOS', 'PAD']) for i in range(pred.shape[1])]))
...
def test(resume, split='test', max_l=100, beam_size=1, use_vq=False, output_topic=False, n=10000, shuffle=False,
    ...
    output_file = 'pred_random_%s%d%s%s_beam%d%s%s%s.txt' % (split, version, resume.split('/')[-1][:-3],
                                                             ...
                                                             '' if mode == 'eval' else '_t',
                                                             '' if repeat_single else '_newsample',
                                                             '' if not sample_lv else (
                                                                 '_lessview' if sample_lv is True else '_oneview'))
...
def encode_sentences(resume, mode='token', split='train', shuffle=False, n=-1, clean_VQ=False, load_VQ=None):
    ...
    tokenizer = Tokenizer(conf['report_dir'], conf['max_l'], file_prefix=conf.get('token_prefix', ''),
    ...
    loader = DataLoader(data, batch_size=conf['valid_batch'], shuffle=shuffle, num_workers=0, drop_last=False)
    sen_tokens = []
    sen_topics = []
    topics = []
    tot = 0
    for (source, targets, debug) in tqdm(loader):
        ...
        if mode == 'token':
            with torch.no_grad():
                tokens, beforeVQ, afterVQ, content = model.module.encode_sentence(targets['find_s'], targets['seg_s'],
                                                                                  ret_VQ=True)
            for i in range(tokens.shape[0]):  # B
                ...
                for j in range(tokens.shape[1]):  # n_sen
                    if tokens[i, j] != 0:
                        ...
                        topics.append((tokens[i, j].cpu().numpy(), beforeVQ[i, j].detach().cpu().numpy(),
                                       afterVQ[i, j].detach().cpu().numpy(), sen, content[i, j].detach().cpu().numpy()))
        else:
            with torch.no_grad():
                _, _, sen_topic = model.module.encode_sentence(targets['find_s'], targets['seg_s'],
                                                               ret_VQ=False)  # [n_sen,B,D]
    ...
    show_points([points[:l], points[l:]],
                ['blue', 'green'], [0.1, 1],
                conf['output_dir'] + '/images/topic_%s_p%d.svg' % (resume.split('/')[-1][:-3], perplexity),
